...
module.exports = async (member) => {
  ...
  if (!inviteMemberData) {
    channel.wsend({ content: `${red} \`${member.user.tag}\` **sunucumuzdan ayrıldı ama kim tarafından davet edildiğini bulamadım.**`});
  } else {
    ...
    channel.wsend({ content:`${red} \`${member.user.tag}\` **sunucumuzdan ayrıldı.** ${inviter.tag} **tarafından davet edilmişti.** [**Toplam ${total} daveti Kaldı**]`});
